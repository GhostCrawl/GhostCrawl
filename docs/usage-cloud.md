...
The Python client is async — every call is awaited inside an `async` function.
...
```python
result = await client.scrape(url="https://example.com", engine="auto")
```
...
mcp = GhostcrawlMCPClient(mcp_url="https://mcp.ghostcrawl.io", api_key="gck_live_...")
await mcp.connect()
...
your current tier, email, and team — the CLI exposes it as
`ghostcrawl auth whoami`, and `ghostcrawl usage` reports your current usage.
